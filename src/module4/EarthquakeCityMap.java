...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(color(255, 0, 0));
		triangle(53, 105, 63, 105, 58, 95);
		fill(color(255, 255, 255));
		ellipse(57, 130, 10, 10);
		fill(color(255, 255, 255));
		rect(53,157,10,10);
		fill(color(255, 255, 0));
		ellipse(57, 220, 10, 10);
		fill(color(0, 0, 255));
		ellipse(57, 250, 10, 10);
		fill(color(255, 0, 0));
		ellipse(57, 280, 10, 10);
		
		fill(0, 0, 0);
		text("City Marker", 75, 100);
		text("Land Quake", 75, 130);
		text("Ocean Quake", 75, 160);
		text("Size ~ Magnitude", 60, 190);
		text("Shallow", 75, 220);
	...
	{
		float oceanCounter = 0;
		//Abraham: More efficient using a hashmap:
		HashMap<String, Float> countryList = new HashMap<String, Float>();
				 ...
				 if(em.isOnLand() == true) {
					 if(country.equals(name)) {
						 if(!countryList.containsKey(country)) {
							 float one = 1;
							 countryList.put(country, one);
						 }
						 else {
							 countryList.put(country, countryList.get(country)+1);
						 }
					 }
				 }
			}
		...
		//This is my own algorithm to calculate the total ocean quakes, as it is much clearer than doing it through the
		//confusing loop above.
		System.out.println(oceanCounter);
		for(String s : countryList.keySet()) {
			System.out.println(s +": " +countryList.get(s) );
		}
		//I used my hashmap and iterated over every earthquake of each country to calculate the total of land quakes.
		float totalLandOnes = 0;
		for (float y : countryList.values()){
			totalLandOnes = totalLandOnes + y;
		}
		System.out.println("total ocean quakes = " + (quakeMarkers.size()-totalLandOnes) );
